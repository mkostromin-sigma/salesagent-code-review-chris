...
# Usage: ./install.sh
# Symlinks (or copies) this checkout to ~/.cursor/plugins/local/salesagent-code-review-chris
set -euo pipefail

HARNESS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLUGIN_NAME="salesagent-code-review-chris"
...
require_file "${HARNESS_DIR}/skills/salesagent-code-review-chris/SKILL.md"
require_file "${HARNESS_DIR}/skills/salesagent-code-review-chris/references/review-charter.md"
require_file "${HARNESS_DIR}/skills/salesagent-code-review-chris/references/reviewer-tooling.md"
require_file "${HARNESS_DIR}/skills/salesagent-code-review-chris/scripts/disposition_ledger.py"
...
detector_count="$(find "${HARNESS_DIR}/skills/salesagent-code-review-chris/scripts" -maxdepth 1 -name '*.py' | wc -l | tr -d ' ')"
...
if [ ! -f "${PWD}/AGENTS.md" ] && [ ! -f "${PWD}/CLAUDE.md" ]; then
  ...
  echo "      Install will succeed; open a salesagent workspace when running /salesagent-code-review-chris." >&2
elif [ ! -d "${PWD}/src" ]; then
...
echo "  agents: ${agent_count}  |  detectors: ${detector_count}  |  skill: salesagent-code-review-chris"
echo
echo "Next steps:"
...
echo "  3. Run:  /salesagent-code-review-chris <PR# | path/glob | (empty = working tree)>"
echo
