...
require_file "${HARNESS_DIR}/skills/full-review/SKILL.md"
require_file "${HARNESS_DIR}/skills/full-review/references/review-charter.md"
require_file "${HARNESS_DIR}/skills/full-review/references/reviewer-tooling.md"
require_file "${HARNESS_DIR}/skills/full-review/scripts/disposition_ledger.py"
...
detector_count="$(find "${HARNESS_DIR}/skills/full-review/scripts" -maxdepth 1 -name '*.py' | wc -l | tr -d ' ')"
...
if [ ! -f "${PWD}/AGENTS.md" ] && [ ! -f "${PWD}/CLAUDE.md" ]; then
  ...
  echo "      Install will succeed; open a salesagent workspace when running /full-review." >&2
elif [ ! -d "${PWD}/src" ]; then
...
echo "  agents: ${agent_count}  |  detectors: ${detector_count}  |  skill: full-review"
echo
echo "Next steps:"
...
echo "  3. Run:  /full-review <PR# | path/glob | (empty = working tree)>"
echo
