...
Repo-fact reference files are bundled at `$HARNESS_ROOT/skills/full-review/references/memory/` (harness plugin root); when an instruction names one, read `$HARNESS_ROOT/skills/full-review/references/memory/<name>.md` with the Read tool. `<MEMORY_DIR>` below means this directory. Citing a filename is not reading it. The orchestrator resolves and passes `HARNESS_ROOT` on every dispatch.
