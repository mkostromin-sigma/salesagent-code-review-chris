...
Repo-fact reference files are bundled at `$HARNESS_ROOT/skills/salesagent-code-review-chris/references/memory/` (harness plugin root); when an instruction names one, read `$HARNESS_ROOT/skills/salesagent-code-review-chris/references/memory/<name>.md` with the Read tool. `<MEMORY_DIR>` below means this directory. Citing a filename is not reading it. The orchestrator resolves and passes `HARNESS_ROOT` on every dispatch.
...
9. **An isolation-worktree `Read` serves MAIN-checkout content, not the worktree's** → an agent working in a sibling/isolated worktree can `Read` a PR-changed file and get the PRE-PR version while `git rev-parse HEAD` in that worktree shows the PR head; a `git checkout <sha>` does NOT make the Read tool follow it. So a specialist can confidently cite findings against code the PR already changed. → In ANY worktree, cite code from `git show HEAD:<path>` / disk-grep (git-object reads are unambiguous), and confirm a **sentinel** — a line you KNOW the PR added/removed — before trusting a bare Read. Also: detectors live in the harness plugin (`$HARNESS_ROOT/skills/salesagent-code-review-chris/scripts/`) and are ABSENT from a fresh salesagent worktree checkout — invoke them by ABSOLUTE `$HARNESS_ROOT/.../scripts/` path with cwd = the tree to scan (reviewer-tooling §H). [surfaced live: 3 of 8 worktree reviewers cited pre-PR content; 2 detectors were unrunnable from the worktree]
