---
name: salesagent-code-review-chris
description: >-
  ...
  the user invokes /salesagent-code-review-chris, asks for a full multi-agent review, or wants
  the salesagent review harness before push/merge.
disable-model-invocation: true
---

# /salesagent-code-review-chris — reviewer-suite orchestrator (MAIN SESSION)
...
The review **target argument** is whatever the user passed after `/salesagent-code-review-chris` or in the same message: a PR number/URL (PR mode), a path/glob (explicit-target mode), or empty (working-tree mode).
...
1. Prefer `~/.cursor/plugins/local/salesagent-code-review-chris` if it exists and contains `skills/salesagent-code-review-chris/references/review-charter.md`.
2. Else, if this skill's files are reachable, take the directory two levels above `skills/salesagent-code-review-chris/` (the plugin / git checkout root).
...
- `$HARNESS_ROOT/skills/salesagent-code-review-chris/scripts/disposition_ledger.py`
...
- `$HARNESS_ROOT/skills/salesagent-code-review-chris/references/review-charter.md`
- `$HARNESS_ROOT/skills/salesagent-code-review-chris/references/reviewer-tooling.md`
- Reference corpus under `$HARNESS_ROOT/skills/salesagent-code-review-chris/references/memory/`: `pr_review_audit_workflow.md`, `gh_actions_dirty_merge_skips_pull_request_workflows.md`, `run_all_tests_congratulations_masks_failures.md`.
...
1. **Explicit path/glob in the user argument** (e.g. `/salesagent-code-review-chris agents/review-*.md` or a salesagent path) — review exactly those. **This is the only way to review the harness suite itself** when those files produce no diff in salesagent.
...
**RE-REVIEW / readiness runs** (`/salesagent-code-review-chris` on a PR already reviewed, or any "is this ready to approve" ask): Dimension A is re-run, not inherited. First: did the head move since your last review? Then re-pull the three endpoints on the CURRENT head. Mechanized gate (charter §4c):

```
python3 $HARNESS_ROOT/skills/salesagent-code-review-chris/scripts/review_completeness.py N --since <your-last-review-or-verdict-time> --head <sha>
```
...
3. Detectors live in `$HARNESS_ROOT/skills/salesagent-code-review-chris/scripts/` — run the whole-tree ones **centrally from the main session** (`recovery_audit`, `sdk_spec_drift`, `citation_freshness`, `ssot_docstring_duplication`) and hand results to the specialists. Tear down sibling worktrees + any `agent-pg-*` containers in Step 7.
