...
salesagent working copy as the workspace, then invoke `/salesagent-code-review-chris`. The agents
...
| Layer | Path | Role |
|---|---|---|
| Orchestrator | `skills/salesagent-code-review-chris/SKILL.md` | `/salesagent-code-review-chris` — runs in the main session, derives the review target, fans out specialists, consolidates. Owns audit dimensions A (review completeness), B (CI), G (quality gates), H (process). |
| Command alias | `commands/salesagent-code-review-chris.md` | Thin `/salesagent-code-review-chris` entry that delegates to the skill. |
...
| Discipline | `skills/salesagent-code-review-chris/references/review-charter.md` | The non-negotiable operating rules, the masking-gotcha doctrine, and the finding/report format. Step-0 read for every agent. |
| Tooling | `skills/salesagent-code-review-chris/references/reviewer-tooling.md` | Detection commands and false-green recipes the charter cites. |
| Detectors (10) | `skills/salesagent-code-review-chris/scripts/*.py` | Mechanized gates: disposition ledger, review completeness, spec-citation freshness, recovery audit, SDK-vs-spec drift, SSOT/duplication, import cycles, FIXME format, suggestion audit, and `bump_check.py`. Each ships a `--selftest` mode. |
| Reference corpus | `skills/salesagent-code-review-chris/references/memory/*.md` | Repo-fact reference files — P1–P42 catalog, wire-envelope policy, BDD harness patterns/pitfalls, AdCP spec-grounding guides, and test-infra gotchas. |

## Requirements

- [Cursor](https://cursor.com) — agents, skills, and `/salesagent-code-review-chris` are Cursor constructs.
...
This symlinks the checkout to `~/.cursor/plugins/local/salesagent-code-review-chris`
...
Then invoke **`/salesagent-code-review-chris` explicitly** — the skill sets
`disable-model-invocation: true`, so Cursor will not auto-start it from ambient
chat; you must type `/salesagent-code-review-chris` (or clearly ask to run the salesagent-code-review-chris skill).

```
/salesagent-code-review-chris 1399                                              # PR by number
/salesagent-code-review-chris https://github.com/prebid/salesagent/pull/1399    # PR by full URL
/salesagent-code-review-chris src/core/tools/                                   # explicit path/glob
/salesagent-code-review-chris                                                   # current working tree
```

| You want… | Invoke |
|---|---|
| Review a PR (first pass or re-review) | `/salesagent-code-review-chris <N>` or `/salesagent-code-review-chris <PR URL>` |
| Review only some paths | `/salesagent-code-review-chris <path/glob>` |
| Review uncommitted local changes | `/salesagent-code-review-chris` |
...
- **Pattern IDs:** bracketed citations in the charter and agents (e.g.
  ...
  stated inline. Files under `skills/salesagent-code-review-chris/references/memory/` are the
