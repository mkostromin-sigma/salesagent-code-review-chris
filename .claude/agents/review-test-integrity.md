...
description: >
  Reviews test QUALITY (non-BDD): does each test prove the code or just the
  mock? Covers mock-only-doesn't-prove-wiring, per-transport wire coverage,
  factory usage (not session.add), no-skip-to-dodge-failures, detached-ORM
  hazards, and the test-infra masking gotchas. Invoke per-PR or on a working-
  tree diff touching tests/ or any src/ behavior with test changes.
color: magenta
tools:
  - Bash
  - Read
